//in this version of Sablier contract, no count of incoming or outgoing streams is tracked,
//so the overall gas consumption for this contract is less
//but the getincomingStreams and getOutgoingStreams loop through total streams two times:
//1. to find the count of incoming streams
//2. to add the incoming streams to the array of struct
//time is traded for optimization of gas
//No bonus question added for this version
...
contract Sablier is ISablier, ReentrancyGuard, CarefulMath {
    ...
    // mapping(user=>tokenAddress=>count) private n_incoming_tokens_user;
    mapping(address => mapping(address => uint256))
        private n_incoming_tokens_per_user;
    ...
    function createStream(
        ...
        n_incoming_tokens_per_user[recipient][tokenAddress] += 1;
        ...
        returns (bool)
    {
        ...
        Types.Stream memory stream = streams[streamId]; // Q1. wourldn't it have been better if it was storage ?
    ...
    {
        //explicit data location for all variables of struct, array or mapping is mandatory
        //does not support returning an array of structs unless enabled ABIencodedv2

        // Types.Stream[] memory incomingStream;
        //cannot push in memory, not possible to resize memory arrays
        ...
        Types.Stream[] memory incomingStream = new Types.Stream[](count);
    ...
    function getOutgoingStreams(address receiver)
        external
        view
        returns (Types.Stream[] memory)
    {
        ...
        for (uint256 i = 100000; i < nextStreamId; i++) {
            Types.Stream storage stream = streams[i];
            if (stream.sender == receiver) {
                count++;
            }
        }

        Types.Stream[] memory outgoingStream = new Types.Stream[](count);
        uint256 j = 0;
        for (uint256 i = 100000; i < nextStreamId; i++) {
            Types.Stream storage stream = streams[i];
            if (stream.sender == receiver) {
    ...
    function getIncomingStreams(
        ...
        uint256 newOffset = offset + n_streams;

        return (newOffset, incomingStream);
    }
    ...
    function getOutgoingStreams(
        ...
        uint256 newOffset;
        newOffset = offset + n_streams;
        return (newOffset, outgoingStream);
    }
}
